...
import { useServerDatafeed } from "@/context/datafeedServerContext";
import { HermesClient } from "@pythnetwork/hermes-client";
import { keccak256, encodeAbiParameters } from "viem";
import { AutomationOrdersABI } from "../../../smartContracts/automationOrders";
import {
...
const AUTOMATIONS = process.env.NEXT_PUBLIC_BASE_AutomationOrders;
...
export default function SessionPlayground() {
  ...
  const { getPriceByMarket } = useServerDatafeed();
  const price = getPriceByMarket("BTC/USD");
  ...
  const step1_ownerNexus = async () => {
    ...
    try {
        ...
        if (!isDeployed) {
            ...
            display("Not V2 account migrated.");
  ...
  const step3_grant = async () => {
    ...
    try {
        ...
        const res = await ownerSession.grantPermissionTypedDataSign({
            ...
            {
                chainId: base.id,
                actionTarget: FUTURES,
                actionTargetSelector: toFunctionSelector(getAbiItem({ abi: FuturesABI, name: "modifyPosition" })),
                actionPolicies: [getSudoPolicy()],
            },
            {
                chainId: base.id,
                actionTarget: AUTOMATIONS,
                actionTargetSelector: toFunctionSelector(getAbiItem({ abi: AutomationOrdersABI, name: "order" })),
                actionPolicies: [getSudoPolicy()],
            },
            {
                chainId: base.id,
  ...
  const step5_enable = async () => {
    ...
    try {
        ...
        const connection = new HermesClient("https://hermes.pyth.network", {}); // TODO: change for private endpoint
        ...
        args: [
            ...
            true,
            true, //isIncrease
            keccak256(encodeAbiParameters([{ type: "string" }], ["BTC/USD"])),
            floatToBigInt(price ? parseFloat(price) : 0),
            Math.round((1 / 100) * 10000),
            updatesToSend,
        ],
        });

        // generate tx data to increasePosition
        const tx1 = {
            to: FUTURES, //futures address
            data: data,
        };
